...
_acl.matmul_acl_i8_create.argtypes = [ctypes.c_int, ctypes.c_int, ctypes.c_int]
_acl.matmul_acl_i8_create.restype = ctypes.c_void_p
_acl.matmul_acl_i8_run.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_int8),
                                ctypes.POINTER(ctypes.c_int32)]
_acl.matmul_acl_i8_run.restype = ctypes.c_int
_acl.matmul_acl_i8_set_B.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_int8)]
_acl.matmul_acl_i8_set_B.restype = ctypes.c_int
...
def matmul_acl_i8(X, W, iters=50):
    """
    ...
    handle = _acl.matmul_acl_i8_create(M, N, K1)
    ret = _acl.matmul_acl_i8_set_B(handle, b_ptr)
    if ret != 0:
        raise RuntimeError(f"matmul_acl_i8_set_B failed, code={ret}")

    # warm-up
    for _ in range(5):
        ret = _acl.matmul_acl_i8_run(handle, a_ptr, c_ptr)
        if ret != 0:
            raise RuntimeError(f"matmul_acl_f16 failed in warmup, code={ret}")
